...
import (
	...
	"github.com/xjasonlyu/tun2socks/v2/router"
...
func netstack(k *Key) (err error) {
	...
	selectedProxy := _defaultProxy
	if len(k.ProxyProcesses) > 0 {
		directProxy, directErr := direct.New()
		if directErr != nil {
			return directErr
		}
		selectedProxy, err = router.NewProcessRouter(_defaultProxy, directProxy, k.ProxyProcesses)
		if err != nil {
			return err
		}
		log.Infof("[ROUTER] proxy only selected processes: %v", k.ProxyProcesses)
	}
	tunnel.T().SetProxy(selectedProxy)
